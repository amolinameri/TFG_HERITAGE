extends Panel
var tower = preload("res://Towers/arrowTower.tscn")
var validTile

func _on_gui_input(event: InputEvent) -> void: # Per click on the map...
	# Arrow Towers cost 50 (for now)
	if GameValues.gold > 49:
			...
			if copyTower:
				...
				for x in range(-2, 3):  
					for y in range(-2, 3): 
						...
						# the only valid tipe of tile is the grass/green we are
						# using from the tilesheet
						var validtile = tileMap.get_cell_atlas_coords(0, tiles)
						
						# is anything from the 5 x 5 is NOT a validTile
						if validtile != Vector2i(3, 5):
							valid = false
		...
		# trow the tower in the map
			...
			var copyTower = get_tree().root.get_node_or_null("copyTower")
			if copyTower:
				#if is not inside the tileMap or in  a valid tile, then we erase the tower
