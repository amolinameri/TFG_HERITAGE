...
var speed = GameValues.arrowSpeedProjectile
...
func _physics_process(delta: float) -> void:
	...
	look_at(targetPosition) # the arrow need to point to the mob
	move_and_slide()

# When the arrow makes contact with the mob, it reduces his health points
# Using his value of damage and then disappear from the map
func _on_area_2d_body_entered(body: Node2D) -> void:
	var mob = body.get_parent()
	if "health" in mob:
		mob.health -= bulletDamage
		queue_free()
